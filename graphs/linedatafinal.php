...
foreach ($scode as $s) {
    ...
    $buydate = $ex[0];
    $fourteenago = date('Y-m-d', strtotime($thisdate. ' - 15 days'));
    ...
    $enddate = date('Y-m-d', strtotime($thisdate. ' - 1 days'));
    ...
    while($x < $jsonlength) {
        ...
        $newpl = $newy2-$y1;
        $percent = round($newpl/$y1*100,2);
        
        $set = array();
        array_push($set,strtotime($thedate));
        array_push($set,$percent);
...
foreach($net as $key => $value) {
    $set = array();
    array_push($set,$key);
    array_push($set,round($value,2));
    array_push($netdata,$set);
}

$datazet = array('name'=>"Total",'color'=>"#000000",'data'=>$netdata);
array_push($total,$datazet);

$fin = json_encode($total, JSON_NUMERIC_CHECK);
print $fin;
?>